...
- While startup refresh or manual all-layer refresh is active, `MapView.tsx` shows a centered `Updating semantic layers...` map overlay with a spinner. This is separate from the per-job RunPod progress overlay.
