...
- On startup, when a backend URL is configured, `App.tsx` automatically attempts the same all-layer RunPod refresh as the manual layer refresh button. `refreshAllScoringLayers()` submits visible layers first in top-to-bottom display order, then hidden layers as background work. Failure is treated as fallback, not as a blocking UI error.
- `MapView.tsx` shows a centered `Updating semantic layers...` overlay while currently visible semantic layers are being attached and no visible semantic content is on the active map yet. Hidden-layer refreshes do not trigger it. If all layer eyes are disabled, the map shows a separate all-layers-hidden overlay and `LayerPanel` pulses the eye buttons.
